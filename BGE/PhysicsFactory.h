...
namespace BGE
{
	...
	public:
		...
		void RigidBody(float x,float y, float z);
		void CreateWall(glm::vec3 startAt, float width, float height, float blockWidth = 5, float blockHeight = 5, float blockDepth = 5);
		void CreateKinenaticWall(glm::vec3 startAt, float width, float height, float blockWidth = 5, float blockHeight = 5, float blockDepth = 5);
		shared_ptr<PhysicsController> CreateKinenaticBox(float width, float height, float depth, glm::vec3 pos, glm::quat quat);
