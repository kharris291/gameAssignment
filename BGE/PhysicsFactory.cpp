...
void PhysicsFactory::RigidBody(float x,float y, float z){
	shared_ptr<PhysicsController> cyl1 = CreateCylinder(0.5f,3, glm::vec3(x, y, z), glm::angleAxis(90.0f, glm::vec3(0,0,0))); //12 2 z
	shared_ptr<PhysicsController> cyl2 = CreateCylinder(0.5f, 3, glm::vec3(x, y+5,z), glm::angleAxis(90.0f, glm::vec3(0,0,0)));
	
	shared_ptr<PhysicsController> box1 = CreateBox(3,6,2, glm::vec3(x+1, y+10, z), glm::quat());

	shared_ptr<PhysicsController> head = CreateSphere(1.0f, glm::vec3(x+1, y+12, z), glm::quat());
	
	btHingeConstraint * hinge = new btHingeConstraint(*cyl1->rigidBody, *cyl2->rigidBody, btVector3(0.0f,2.0f,0.0f),btVector3(0.0f,-2.0f,0.0f), btVector3(1,0,0), btVector3(1,0,0), true);
	//right leg
	dynamicsWorld->addConstraint(hinge);

	hinge = new btHingeConstraint(*box1->rigidBody, *cyl2->rigidBody, btVector3(-1.0f,-3.0f,0.0f),btVector3(0.0f,2.0f,0.0f), btVector3(2,0,0), btVector3(1,0,0), true);
	
	dynamicsWorld->addConstraint(hinge);


	cyl1 = CreateCylinder(0.5f, 3, glm::vec3(x-2, y+5, z), glm::angleAxis(90.0f, glm::vec3(0,0,0))); 
	cyl2 = CreateCylinder(0.5f, 3, glm::vec3(x-2, y+10,z), glm::angleAxis(90.0f, glm::vec3(0,0,0)));
	hinge = new btHingeConstraint(*cyl1->rigidBody, *cyl2->rigidBody, btVector3(0.0f,2.0f,0.0f),btVector3(0.0f,-2.0f,0.0f), btVector3(2,0,0), btVector3(1,0,0), true);
	//right arm
	dynamicsWorld->addConstraint(hinge);

	hinge = new btHingeConstraint(*box1->rigidBody, *cyl2->rigidBody, btVector3(-2.5f,3.0f,0.0f),btVector3(0.0f,2.0f,0.0f), btVector3(2,0,0), btVector3(1,0,0), true);
	
	dynamicsWorld->addConstraint(hinge);


	cyl1 = CreateCylinder(0.5f, 3, glm::vec3(x+2, y, z), glm::angleAxis(90.0f, glm::vec3(0,0,0))); 
	cyl2 = CreateCylinder(0.5f, 3, glm::vec3(x+2, y+5,z), glm::angleAxis(90.0f, glm::vec3(0,0,0)));
	hinge = new btHingeConstraint(*cyl1->rigidBody, *cyl2->rigidBody, btVector3(0.0f,2.0f,0.0f),btVector3(0.0f,-2.0f,0.0f), btVector3(2,0,0), btVector3(1,0,0), true);
	//left leg
	dynamicsWorld->addConstraint(hinge);
	...
	cyl1 = CreateCylinder(0.5f, 3, glm::vec3(x+4, 7, z), glm::angleAxis(90.0f, glm::vec3(0,0,0))); 
	cyl2 = CreateCylinder(0.5f, 3, glm::vec3(x+4, 12,z), glm::angleAxis(90.0f, glm::vec3(0,0,0)));
	hinge = new btHingeConstraint(*cyl1->rigidBody, *cyl2->rigidBody, btVector3(0.0f,2.0f,0.0f),btVector3(0.0f,-2.0f,0.0f), btVector3(2,0,0), btVector3(1,0,0), true);
	//left arm
	dynamicsWorld->addConstraint(hinge);
	...
	hinge = new btHingeConstraint(*box1->rigidBody, *head->rigidBody, btVector3(0.0f,3.0f,0.0f),btVector3(0.0f,-1.0f,0.0f), btVector3(2,0,0), btVector3(1,0,0), true);
	
	dynamicsWorld->addConstraint(hinge);
}

void PhysicsFactory::CreateKinenaticWall(glm::vec3 startAt, float width, float height, float blockWidth, float blockHeight, float blockDepth)
{
	float z = startAt.z;
	float gap = 1;
	...
	for (int w = 0 ; w < width ; w ++)
	{
		for (int h = 0 ; h < height ; h ++)	
		{
			float x = startAt.x + ((blockWidth + 2) * w);
			float y = ((blockHeight + gap) / 2.0f) + ((blockHeight + gap) * h);
			CreateKinenaticBox(blockWidth, blockHeight, blockDepth, glm::vec3(x, y, z), glm::quat());
		}
	}
}
...
shared_ptr<PhysicsController> PhysicsFactory::CreateKinenaticBox(float width, float height, float depth, glm::vec3 pos, glm::quat quat)
{
	// Create the shape
	btCollisionShape * boxShape = new btBoxShape(btVector3(width, height, depth) * 0.50);
	btScalar mass = 0;
